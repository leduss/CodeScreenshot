...
describe('export-limit storage helpers', () => {
  ...
  it('persists and reads from localStorage + cookie', () => {
    ...
    expect(window.localStorage.getItem('snapcode_exports_used')).toBe('3');
    expect(document.cookie.includes('snapcode_exports_used=3')).toBe(true);
    expect(readStoredExportsUsed()).toBe(3);
  });

  it('reads value from cookie when localStorage missing', () => {
    document.cookie = 'snapcode_exports_used=4; path=/';
    window.localStorage.removeItem('snapcode_exports_used');
